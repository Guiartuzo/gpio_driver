...
This gives us the physical address to the gpio peripheral, but we don't want use this hardcoded address...instead we should use the <pre>```gpiod_get()```/pre> which is part of the Linux GPIO descriptor API. By using this function we can guarantee portability between rasp models and future proof as well (in case the address changes in the future), by using the `ioremap(0x3f20000, size);` we are ignoring concurrency and power management which can cause conflicts with other drivers and GPIO users.
